...
var settings = require('./vidStreamerConf.json');
...
var mimeTypes = _.extend({
	...
	".ts": "video/mp2t",
	".ogg": "video/ogg"
},settings.customMimes||{});

var vidStreamer = function (req, res, path,fsErrorCbk) {
	var stream;
	var stat;
	var info = {};
	var ext;
	...
	info.path = path || (typeof reqUrl.pathname === "string" ? reqUrl.pathname.substring(1) : undefined);
	if (info.path) {
		try {
	...
	if (!info.path) {
		handler.emit("fileCheck", res, fsErrorCbk);
		return false;
	}
		...
		console.log(info.path+" not a substring of " + settings.rootPath)
		handler.emit("security", res, { message: info.path });
		return false;
	} else {
		ext = info.path.match(/.*(\..+?)$/);
	...
	try {
		...
		handler.emit("fileCheck", res, fsErrorCbk, e);
		return false;
	}
...
handler.on("fileCheck",function(res,fsErrorCbk,e){
	if(fsErrorCbk instanceof Function) fsErrorCbk();
	else handler.emit("badFile",res,fsErrorCbk||e);
});
